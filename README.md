...
Many of the above features can be emulated in languages with what I would call **weaker** support for FP. Consider Python. Yes, you can defune just functions in Python. Yes, you can even define curried functions with **effort**. Yes, one can even emulate custom operators and write code similar to the above with **effort**. But it takes effort! This is consistent with my introductory point that functional programming can be done in any language or put another way functional patterns and concepts can find an incarnation via any programming language. However, if things are too unnatural or difficult developers aren't likely to do them. Really, we won't! Hence, programming languages do affect things like design and style to substantial degree.
   
* Q: Why code in this weird language with weird syntax?
    - Not sure who initially came up with this gem: "There is no such thing as intuitive. There's only the familiar". Really, truely deep.
    ...
    - The real point is that advanced languages have wonderfully powerful type systems and semantics which support FP well and consequently yield productivity gains.
...
* Q: But didn't you say that OO and FP are complementary? It seems you are arguing against OO here. 
    - Not really.
...
Typeclass is not a well defined term. Most folks consider Haskell's implementation to be the gold standard. One could coceive of it as a both a pattern and a mechanism for unifying types at compile time. A good resource detailing a translation of Haskell's semantics into OCaml can be found [here](http://okmij.org/ftp/Computation/typeclass.html). Another way to think about it is that Typeclasses achieve a simmilar effect that polymorphism does in object programming.

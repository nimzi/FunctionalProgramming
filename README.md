...
Many attempts at explaining FP do so by contrasting it with the current status quo (object programming) , however, I believe this is a wrong pathway to put an inquisitive reader on. FP and OO for example are more complementary than they are conflicting and the **modern** programming languages such as Swift, Scala, Rust, OCaml (and F#) along with the **modernized** Javascript (and its improvement - the Typescript), the Javas and C#s of the world are all headed in the direction of adopting features that well support functional programming. It might have been Martin Odersky (the creator of Scala) that coined a term **postfunctional** that is a decent tag for all this innovation in programming languages. 
